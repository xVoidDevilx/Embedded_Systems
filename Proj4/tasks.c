...
void TSKPayload(void * arg0)
{
    ...
    uint32_t gatekey;           //lock access to resources
        ...
        for(;;) //Ever
        {
            ...
            if (glo.PayloadQueue.DroppedMessage)
            {
                glo.PayloadQueue.DroppedMessage = false;        //reset this flag
                UART_write(glo.DEVICES.uart0, "Queue Overflow\n\r", strlen("Queue Overflow\n\r"));
            }
