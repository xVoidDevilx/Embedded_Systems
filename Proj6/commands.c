...
void PrintAbout(char* OUTPUT, size_t buffer_size) {
    ...
    int chars_made = snprintf(OUTPUT, space_left, "\r\n\tEngineer: %20s"
                            "\n\r\tDate | Time: %13s | %s"
                            "\n\r\tVersion: %9.1f"
                            "\n\r\tAssignment %d: %12s\n\r", "Silas Rodriguez",__DATE__, __TIME__, 0.6, 6,"Registers");
...
void RegCMD(char *input)
{
    ...
    switch(*token)
    {
        ...
        case '>':
            glo.REGISTERS[reginx] = litsrc0 > litsrc1 ? 1:0;
            break;
        // min or less than op
        case '<':
            glo.REGISTERS[reginx] = litsrc0 < litsrc1 ? 1:0;
...
int32_t ExtractLitSrc (char * token)
{
    ...
    switch (*token)
    {
        ...
        case '#':
            ...
            // check what type of literal being passed: hex or dec
            if (strncmp(token, "0x", 2) ==0 || strncmp(token, "0X", 2) == 0)
                litsrc = (int32_t) strtoul(token, &ptr, 16);      // pass the number as base 16
            else
                litsrc = (int32_t) strtoul(token, &ptr, 10);      // pass the number as a decimal
            break;
