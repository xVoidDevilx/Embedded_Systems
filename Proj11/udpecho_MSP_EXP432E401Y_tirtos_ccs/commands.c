...
void PrintAbout() {
    ...
    sprintf(txbuf, "\r\n\tEngineer: %-20s"
                    ...
                    "\n\r\tAssignment %-2d: %-18s"
                    "\n\r\tIP: %3d.%3d.%3d.%3d\n\r",
                    "Silas Rodriguez", __DATE__, __TIME__, 1.2, 12,"NETUDP",
...
}   // end of NetUDP out command

//void Dial(char *message)
//{
//    // IP_SHADOW_REG -> SHADOW_DIAL_IP Macro
//    char *token;
...
//    if(!token)
//    {
//        if(glo.SHADOWS[SHADOW_DIAL_IP] != 0)
//        {
//            sprintf(netbuf, "-netudp %d.%d.%d.%d:%d",
//                    (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP] >> 24)&0xFF, (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP] >> 16)&0xFF,
//                    (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP] >> 8)&0xFF,(uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP])&0xFF);
//            sprintf(txbuf, "%s -stream 0", netbuf);
//            AddPayload(txbuf);
//            AddPayload("-stream 0");
//            sprintf(txbuf, "%s -reg %d #0", netbuf, SHADOW_DIAL_IP);
//            AddPayload(txbuf);
//        }
//        glo.SHADOWS[SHADOW_DIAL_IP] = 0;
//        glo.SHADOWS[SHADOW_DIAL_IP+1] = DEFAULT_NET_PORT;
//        goto PRINT_ADDR;
//    }
//
//    if(*token == 'r')
...
//
//    glo.SHADOWS[SHADOW_DIAL_IP] = (clientAddr.sin_addr.s_addr & 0xFF)  << 24;
//    glo.SHADOWS[SHADOW_DIAL_IP] |= (clientAddr.sin_addr.s_addr & 0xFF) << 8;
//    glo.SHADOWS[SHADOW_DIAL_IP] |= (clientAddr.sin_addr.s_addr & 0xFF) >> 8;
//    glo.SHADOWS[SHADOW_DIAL_IP] |= (clientAddr.sin_addr.s_addr & 0xFF) >> 24;
//
//    glo.SHADOWS[SHADOW_DIAL_PORT] = (clientAddr.sin_port & 0xFF)    << 8;
//    glo.SHADOWS[SHADOW_DIAL_PORT] |= (clientAddr.sin_port & 0xFF00) >> 8;
//
//    sprintf(netbuf, "-netudp %d.%d.%d.%d:%d",
//            (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP]>>24)&0xFF, (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP]>>16)&0xFF,
//            (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP]>>8)&0xFF, (uint8_t)(glo.SHADOWS[SHADOW_DIAL_IP])&0xFF);
//
//    sprintf(txbuf, "%s -reg %d #%d", netbuf, SHADOW_DIAL_IP, glo.SHADOWS[SHADOW_NET_IP]);
//    AddPayload(txbuf);
//    sprintf(txbuf, "%s -reg %d #%d", netbuf, SHADOW_DIAL_PORT, glo.SHADOWS[SHADOW_NET_IP+1]);
//    AddPayload(txbuf);
//    sprintf(txbuf, "%s -script 0 lq audio", netbuf);
//    AddPayload(txbuf);
//    AddPayload("-script 0 lq audio");
//    sprintf(txbuf, "%s -script 0 X", netbuf);
//    AddPayload(txbuf);
//    AddPayload("-script 0 X");
//
//PRINT_ADDR:
//
//}
