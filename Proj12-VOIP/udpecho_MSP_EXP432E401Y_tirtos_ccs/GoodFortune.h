...
// MACROS
#define MAXLEN 300       // 300 Char Payloads max
...
/* Phone call Macros */
#define IP_SHADOW_REG       26
#define PORT_SHADOW_REG     27
#define SHADOW_DIAL_0_IP    28
...
typedef struct _message {
    uint32_t index;              //current position in the buffer
    //uint32_t MsgCount;         // This messages current index
    char msgBuf[MAXLEN];        // the buffer for this message
}Message;

typedef struct _messageQueue {
    uint32_t MsgReading;     //where they come out
    uint32_t MsgWriting;     //where im writing
...
void CallbackCMD(char *message);                                        // Function to configure callbacks
...
void Dial(char *message);                                               // dial a IP address for VOIP
