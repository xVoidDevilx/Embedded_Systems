...
void HelpCMD(char* command)
{
    ...
    if (token == NULL)
    {
        ...
        {
            System_snprintf(buffer, MAXLEN<<1,
                "%-8s | %s\n\r",
                glo.COMMANDS[i]->name,
...
void PrintAbout() {
    ...
    sprintf(txbuf, "\r\n\tEngineer: %-20s"
                    ...
                    "\n\r\tAssignment %-2d: %-18s"
                    "\n\r\tIP: %3d.%3d.%3d.%3d\n\r",
                    "Silas Rodriguez", __DATE__, __TIME__, 1.2, 13,"VOIP",
    ...
    UART_write(glo.DEVICES.uart0, txbuf, strlen(txbuf));
}
...
void MemrCMD(char *addrHex) {
    ...
    if (memaddr > 0x100000 && memaddr < 0x20000000) goto MEMERR;    // too high for flash, too low for SRAM
        ...
        for (i = 0; i <= 0xF; i+=4) {
            ...
            snprintf(buffer, MAXLEN<<1, "Address 0x%08X: 0x%08x\n\r",
                     memaddr + i, value);
            // write out
            UART_write(glo.DEVICES.uart0, buffer, strlen(buffer));
        }
    ...
    MEMERR:
        ...
        System_snprintf(buffer, MAXLEN<<1,
                        "Hex address %s out of allowable range",
                        addrHex);
        ErrorOut(&BADMEM, buffer);
    }
}

//Tokenize the incoming string, assign the callback with index to a new payload - working!
void CallbackCMD(char *message)
{
    char service[MAXLEN] = {0};
    char msgbuf[MAXLEN] = {0};
    // Parse the input string to get callback parameters
    char *token = GetNxtStr(message, true), *endptr;  //  -callback [id] [times] [payload] - > [id] [times] [payload]
    uint32_t i = 0;
    //print all callbacks
    if (token == NULL)
    {
        for(i = 0; i<NUMCALLBACKS; i++){
            System_snprintf(msgbuf, MAXLEN, "Callback [%d] | Count: [%d] | Payload: %s\n\r",
    ...
    if(!strncmp(token, "clear", strlen("clear")) || *token == 'c' || *token == 'C')
    {
        for(i = 0; i<NUMCALLBACKS; i++)
        {
    ...
    if (index < 0 || index >= NUMCALLBACKS)
    {
        ...
        System_snprintf(msgbuf, MAXLEN, "Invalid callback index. Must be between 0 and %d\n\r", NUMCALLBACKS - 1);
        ErrorOut(&BADGPIO, msgbuf);
        return;
    }
...
void TickerCMD(char *input)
{
    ...
    if(token == NULL)
    {
        ...
        {
            snprintf(buffer, MAXLEN, "TICKER %d: d: %d | p: %d | c: %d | payload: %s\n\r",
    ...
    if(token == NULL)
    {
        System_snprintf(buffer, MAXLEN, "TICKER %d: d: %d | p: %d | c: %d | payload: %s\n\r",
                        i, glo.tickers[i].delay, glo.tickers[i].period, glo.tickers[i].count, glo.tickers[i].payload);
        UART_write(glo.DEVICES.uart0, buffer, strlen(buffer));
        return;
    }
...
void RegCMD(char *input)
{
    ...
    if(token == NULL)
    {
        ...
        gatekey = GateSwi_enter(glo.BIOS->RegsGate);    // lock before read
        for (i=0; i<NUMREG; i+=2)
        {
            snprintf(buffer, MAXLEN, "-print REG %2d: %11d (0x%08X) | REG %2d: %11d (0x%08X)",
                     i, glo.REGISTERS[i], glo.REGISTERS[i], i+1,
                     glo.REGISTERS[i+1], glo.REGISTERS[i+1]);
            AddPayload(buffer);
        }
        GateSwi_leave(glo.BIOS->RegsGate, gatekey);
        return;
    }
    //reset registers
    else if (*token == 'c')
    {
        gatekey = GateSwi_enter(glo.BIOS->RegsGate);
        for (i=0; i<NUMREG; i++)
            glo.REGISTERS[i] = 0;

        AddPayload("-print Registers Cleared Sucessfully");
        GateSwi_leave(glo.BIOS->RegsGate, gatekey);
        return;
    }
    // Parse for a dst
    ...
    if (endptr == NULL)
    {
        ...
        REGEXIT:
        snprintf(buffer, MAXLEN, "-print REG %2d: %d (0x%08X)",
                 reginx, glo.REGISTERS[reginx],glo.REGISTERS[reginx]);
...
void ScriptCMD(char *input)
{
    ...
    if (token == NULL)
    {
        ...
        {
            System_snprintf(buffer, MAXLEN, "Script %d: %s\n\r",
                            i, glo.scripts[i]);
            UART_write(glo.DEVICES.uart0, buffer, strlen(buffer));
        }
        return;
...
void NetUDPCmd(char *message, int32_t binaryCount)
{
    ...
    gatekey = GateSwi_enter(glo.BIOS->UDPOutWriteGate);
    memcpy(glo.netOutQueue.payloads[glo.netOutQueue.PayloadWriting], token, strlen(token) + binaryCount + 1);
    glo.netOutQueue.BinaryCount[glo.netOutQueue.PayloadWriting] = binaryCount;
    payloadnext = glo.netOutQueue.PayloadWriting + 1;

    if (payloadnext >= QUEUELEN)
        payloadnext = 0;
    glo.netOutQueue.PayloadWriting = payloadnext;

    GateSwi_leave(glo.BIOS->UDPOutWriteGate, gatekey);
    Semaphore_post(glo.BIOS->UDPOutSem);
}   // end of NetUDP command
