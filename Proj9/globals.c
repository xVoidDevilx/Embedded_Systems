...
void GlobalConfig(global *glo, UART_Handle uart0, UART_Handle uart7,Timer_Handle timer0, Timer_Handle ticker){
    ...
    strcpy(glo->scripts[0], "-rem this is a sample script");
    strcpy(glo->scripts[1], "-gpio 0 t");
    strcpy(glo->scripts[2], "-gpio 1 t");
    strcpy(glo->scripts[3], "-gpio 2 t");
    strcpy(glo->scripts[4], "-gpio 3 t");

    strcpy(glo->scripts[6], "-rem this script configs callbacks");
    strcpy(glo->scripts[7], "-timer 0");
    strcpy(glo->scripts[8], "-callback 1 -1 -regs 31 ++");
    strcpy(glo->scripts[9], "-callback 2 -1 -regs 31 --");
    strcpy(glo->scripts[10], "-callback 0 10 -print Timer popped");

    strcpy(glo->scripts[12], "-rem this configs tickers for binary counter");
    strcpy(glo->scripts[13], "-ticker 12 20 20 -1 -gpio 0 t");
    strcpy(glo->scripts[14], "-ticker 13 40 40 -1 -gpio 1 t");
    strcpy(glo->scripts[15], "-ticker 14 80 80 -1 -gpio 2 t");
    strcpy(glo->scripts[16], "-ticker 15 160 160 -1 -gpio 3 t");

    strcpy(glo->scripts[18], "-rem this tests conditionals");
    strcpy(glo->scripts[19], "-if #2 > #3 ? -scripts 0 X:");
    strcpy(glo->scripts[20], "-if 31 > #0 ? -scripts 0 X:");
    strcpy(glo->scripts[21], "-if 31 < #0 ? -scripts 1 x:");
    strcpy(glo->scripts[22], "-if 31 = #0 ? -scripts 12 X:");
