...
void PrintAbout(char *OUTPUT, size_t buffer_size)
{
    ...
    int chars_made = snprintf(OUTPUT, space_left, "\r\n\tEngineer: %20s"
                                                  ...
                                                  "\n\r\tVersion: %9.1f"
                                                  "\n\r\tAssignment %d: %12s\n\r",
                              "Silas Rodriguez", __DATE__, __TIME__, 1.0, 11, "Local Voice");
...
void MemrCMD(char *addrHex)
{
    ...
    if (memaddr > 0x100000 && memaddr < 0x20000000)
        goto MEMERR; // too high for flash, too low for SRAM
    else if (memaddr > 0x20040000 && memaddr < 40000000)
        goto MEMERR; // too high for SRAM too low for peripheral
    else if (memaddr > 0x44055000)
        goto MEMERR; // above peripherals
    else
    {
        int i;
...
/**
 * @brief Sends a command via UART7.
 *
...
/**
 * @brief Processes voice command.
 *
 * @param payload String containing voice command information.
 */
void VoiceCMD(char *payload)
{
    ...
    uint16_t *dest;
    char *loc;
    // Retrieve destination choice from payload
    loc = GetNxtStr(payload, true);
    if (loc == NULL)
        return;
    dest_choice = atoi(loc);

    // Extract destination choice and buffer length information
    loc = GetNxtStr(loc, true);
    bufflen = atoi(loc);

    // Check buffer length and increment error count if necessary
    if (bufflen != DATABLOCKSIZE)
        VoiceERR.count++;

    // Move to the data section in the payload
    loc = GetNxtStr(loc, false);
    loc++;
    loc++;

    // Choose the destination based on the given choice
    if (dest_choice == 0)
        dest = glo.TxRx.TX_Ping;
    else if (dest_choice == 1)
        dest = glo.TxRx.TX_Pong;

    // Copy data to the selected destination
    memcpy(dest, loc, sizeof(uint16_t) * DATABLOCKSIZE);

    // Update ADC buffer control if necessary
...
/**
 * @brief Processes streaming command.
 *
 * @param message String containing streaming command.
 */
void StreamCMD(char *message)
{
    ...
    // Determine the intent based on the token or ADC buffer control state
    if (token == NULL)
    {
        // Token was empty; decide intent based on the current state of the ADC buffer control
        switch (glo.DEVICES.adcbufctrl.converting)
        {
        // Different states and their corresponding intent values
        case 0:
            intent = 1; // Transition to intent 1
            break;
        case 1:
            intent = 2; // Transition to intent 2
            break;
        case 2:
            intent = 0; // Transition to intent 0
            break;
        }
    }
    else
    {
        // Token contains an intent; determine based on the received token
        switch (*token)
        {
        // Different cases based on the received token content
        case '2':
            intent = 2;
            break;
        case '1':
    ...
    // If the intended state matches the current state, no action needed, return
    if (intent == glo.DEVICES.adcbufctrl.converting)
        return;

    // Transition logic based on the determined intent
    if (intent == 0)
    {
        // Handling for intent 0: Stop current process
        if (glo.DEVICES.adcbufctrl.converting == 2)
        {
            // Cancel ADC conversion if currently converting
            if (ADCBuf_convertCancel(glo.DEVICES.adcBuf) != ADCBuf_STATUS_SUCCESS)
                ErrorOut(&StreamERR, "ADC cancel failed");
        }

        // Reset ADC buffer control and related elements
        glo.DEVICES.adcbufctrl.converting = 0;
        if (glo.LUTctrl.deltalut > 0.)                                            // To verify the logic in this condition
            sineCMD("-sine 0");                                                   // Trigger sine command
        CallbackCMD("-callback 0 c", glo.terminal_out, sizeof(glo.terminal_out)); // Execute callback command
        glo.DEVICES.adcbufctrl.RX_completed = NULL;
        glo.DEVICES.adcbufctrl.TX_completed = NULL;
        glo.DEVICES.adcbufctrl.TX_index = -1;
    }
    // Start microphone streaming process
    else if (intent == 1 && glo.DEVICES.adcbufctrl.converting == 0)
    {
        // Start the microphone stream process
        GPIOCMD("-gpio 4 w 0");                                 // Disable speaker
        GPIOCMD("-gpio 5 w 1");                                 // Enable microphone
        TimerCMD("-timer 125");                                 // Set timer
        glo.DEVICES.adcbufctrl.converting = 1;                  // Update conversion state
        TickerCMD("-tickers 14 100 0 0 -stream 2");             // Trigger stream-related ticker command
        TickerCMD("-tickers 15 105 0 0 -callback 0 -1 -audio"); // Execute audio-related ticker command
    }
    // Start data conversion process
    else if (intent == 2 && glo.DEVICES.adcbufctrl.converting == 1)
    {
        // Start ADC conversion process if conditions met
        if (ADCBuf_convert(glo.DEVICES.adcBuf, &glo.DEVICES.adcbufctrl.conversion, 1) != ADCBuf_STATUS_SUCCESS)
            ErrorOut(&ADCERR, "ADC Convert Failed"); // Handle conversion failure
        else
        {
            // Update ADC buffer control upon successful conversion
...
/**
 * @brief Processes audio command.
 *
 * @param message String containing audio command.
 */
void AudioCMD(char *message)
{
    ...
    // Check if data transmission is pending or delayed
    if (glo.DEVICES.adcbufctrl.TX_completed != NULL && glo.DEVICES.adcbufctrl.TX_index >= 0 && glo.DEVICES.adcbufctrl.delay > 0)
    {
        glo.DEVICES.adcbufctrl.delay--;
        return;
    }
    // Extract the value to be transmitted
    outval = glo.DEVICES.adcbufctrl.TX_completed[glo.DEVICES.adcbufctrl.TX_index++];

    // Configure SPI transaction settings
    spiTrans.count = 1;
    spiTrans.txBuf = (void *)&outval;
    spiTrans.rxBuf = (void *)NULL;

    // Initiate transfer out to speaker
    ...
    // Check if the entire data block has been transmitted
